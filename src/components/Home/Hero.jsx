    ...
    <div className='flex flex-col items-center w-full min-h-screen bg-secondary '>
      <div className='w-full flex flex-col md:flex-row  md:items-center md:w-[80%] md:h-[80vh]'>
        <div className='flex flex-col items-center gap-4 p-4'>
          <h1 className='text-[2.5rem] font-jost font-bold text-primary md:text-[3rem]'>Your Health is Our Top Priority </h1>
          <div>
        ...
        <div className='w-[80%] bg-primary  flex flex-col justify-between md:flex-row gap-2 items-center '>
          {successHistory.map((success, i) => (
            <div key={i} className='w-[70%] md:w-[20%] p-4 flex flex-col items-center text-center'>
              <h3 className='text-[2rem] md:text-[1.8rem] text-white font-jost'>{success.value}</h3>
              <p className='text-white md:text-[1.2rem]'>{success.name}</p>
            </div>
          ))}
        </div>
      </div>
